...
impl Agents {
    ...
    fn has(&self, agent_id: usize) -> bool {
        agent_id < self.agents.len() && self.agents[agent_id].is_alive()
    }

    fn get(&self, agent_id: usize) -> Option<&Agent> {
        if !self.has(agent_id) {
            return None;
        }
        Some(&self.agents[agent_id])
    }

    fn add(&mut self, agent: Agent) -> usize {
        if self.availables.len() > 0 {
            let index = self.availables.remove(0);
            println!("create availables-------{}", index);
...
impl AgentIdAccessor {
    ...
    pub fn borrow_ids<T: Component>(&mut self, manager: &AgentManager) -> Option<&Vec<usize>> {
        ...
        if needs_update {
            let src = &manager.borrow_agent_ids::<T>().unwrap();
    ...
    pub fn borrow_ids_for_pair<
        T1: Component,
        T2: Component
    >(&mut self, manager: &AgentManager) -> Option<&Vec<usize>> {
        ...
        if needs_update {
            ...
            let src = &manager.borrow_agent_ids::<T1>().unwrap();
...
impl AgentManager {
    ...
    fn get_updated_frame<T: Component>(&self) -> u64 {
    ...
    pub fn get_agent(&mut self, agent_id: usize) -> Option<&Agent> {
    ...
    pub fn add_component_to_agent<T: Component>(&mut self, agent_id: usize, component: T) -> &mut Self {
        ...
        self.borrow_component_manager_mut::<T>()
            .add(agent_id, component);
        self.updated_frame_map.insert(TypeId::of::<T>(), self.get_frame());

        self
    }

    fn borrow_agent_ids<T: Component>(&self) -> Option<&Vec<usize>> {
        if ! self.has_component_manager::<T>() {
            // @TODO: Better error handling
            println!("Unknown component");
            return None;
        }
        Some(self.borrow_component_manager::<T>().borrow_agent_ids())
    }

    pub fn borrow_component<T: Component>(&self, agent_id: usize) -> Option<&T> {
        match self.has_component_manager::<T>() {
            true => self.borrow_component_manager::<T>()
                .borrow_component(agent_id),
            false => None
        }
    }

    pub fn borrow_component_mut<T: Component>(&mut self, agent_id: usize) -> Option<&mut T> {
        match self.has_component_manager::<T>() {
            true => self.borrow_component_manager_mut::<T>()
                .borrow_component_mut(agent_id),
            false => None
        }
    }

    pub fn borrow_components<T: Component>(&self) -> Option<&Vec<T>> {
        match self.has_component_manager::<T>() {
            true => Some(
    ...
    pub fn borrow_components_mut<T: Component>(&mut self) -> Option<&mut Vec<T>> {
        match self.has_component_manager::<T>() {
            true => Some(
                self.borrow_component_manager_mut::<T>()
    ...
    pub fn borrow_component_pair_mut<
        T1: Component,
        T2: Component
    >(&mut self, agent_id: usize) -> Option<(&mut T1, &mut T2)> {
        if ! self.has_component_manager::<T1>() ||
            ! self.has_component_manager::<T2>() {
            return None;
        }
        ...
        if !manager1.has(agent_id) || !manager2.has(agent_id) {
            return None;
        }

        Some((
            manager1.borrow_component_mut(agent_id).unwrap(),
            manager2.borrow_component_mut(agent_id).unwrap()
        ))
    }
    ...
    pub fn borrow_component_triple_mut<
        ...
        T2: Component,
        T3: Component
    >(&mut self, agent_id: usize) -> Option<(&mut T1, &mut T2, &mut T3)> {
        if ! self.has_component_manager::<T1>() ||
            ! self.has_component_manager::<T2>() ||
            ! self.has_component_manager::<T3>() {
            return None;
        ...
        if !manager1.has(agent_id) || !manager2.has(agent_id) || !manager3.has(agent_id) {
            return None;
        }
        ...
        Some((
            manager1.borrow_component_mut(agent_id).unwrap(),
            manager2.borrow_component_mut(agent_id).unwrap(),
            manager3.borrow_component_mut(agent_id).unwrap()
        ))
    }
    ...
    pub fn borrow_component_quad_mut<
        ...
        T3: Component,
        T4: Component
    >(&mut self, agent_id: usize) -> Option<(&mut T1, &mut T2, &mut T3, &mut T4)> {
        if ! self.has_component_manager::<T1>() ||
            ! self.has_component_manager::<T2>() ||
            ! self.has_component_manager::<T3>() ||
            ! self.has_component_manager::<T4>() {
        ...
        if !manager1.has(agent_id) || !manager2.has(agent_id) || !manager3.has(agent_id) || !manager4.has(agent_id) {
            return None;
        }
        ...
        Some((
            ...
            manager2.borrow_component_mut(agent_id).unwrap(),
            manager3.borrow_component_mut(agent_id).unwrap(),
            manager4.borrow_component_mut(agent_id).unwrap()
        ))
    }

    fn has_component_manager<T: Component>(&self) -> bool {
        let type_id = TypeId::of::<T>();
        self.manager_map.contains_key(&type_id)
    }

    fn borrow_component_manager<T: Component>(&self) -> &ComponentManager<T> {
        let type_id = TypeId::of::<T>();
        cast_manager(self.manager_map.get(&type_id).unwrap().as_ref())
    }

    fn borrow_component_manager_mut<T: Component>(&mut self) -> &mut ComponentManager<T> {
        let type_id = TypeId::of::<T>();
...
// @TODO: Write comment
fn cast_manager_mut_unsafe<T: Component>
